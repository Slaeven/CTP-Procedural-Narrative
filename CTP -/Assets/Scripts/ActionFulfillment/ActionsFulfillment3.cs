...
public class ActionsFulfillment3 : MonoBehaviour
{
    #region Variables
    public string[,] acts;
    public string[] lines;
	...
	void Update ()
    {
        if (waitComplete)
        {
            if (System.IO.File.Exists(Application.dataPath + "/output4.txt")/* && !doneOnce3*/)
            {
                //doneOnce3 = true;
    ...
    IEnumerator WaitforConstruct()
    {
        ...
        taskComplete = true;
        waitComplete = true;
        acts = this.gameObject.GetComponent<AINavigation3>().acts;
        lines = this.gameObject.GetComponent<AINavigation3>().lines;
        safeHouseArray = GameObject.FindGameObjectsWithTag("SafeHouse");
        string path1 = Application.dataPath + "/output4.txt";
