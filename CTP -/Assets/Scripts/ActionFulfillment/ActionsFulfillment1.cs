...
public class ActionsFulfillment1 : MonoBehaviour
{
    #region Variables
    public string[,] acts;
    public string[] lines;
	...
	void Update ()
    {
        if (waitComplete)
        {
            if (System.IO.File.Exists(Application.dataPath + "/output2.txt")/* && !doneOnce3*/)
            {
                //doneOnce3 = true;
    ...
    IEnumerator WaitforConstruct()
    {
        ...
        taskComplete = true;
        waitComplete = true;
        acts = this.gameObject.GetComponent<AINavigation1>().acts;
        lines = this.gameObject.GetComponent<AINavigation1>().lines;
        safeHouseArray = GameObject.FindGameObjectsWithTag("SafeHouse");
        string path1 = Application.dataPath + "/output2.txt";
