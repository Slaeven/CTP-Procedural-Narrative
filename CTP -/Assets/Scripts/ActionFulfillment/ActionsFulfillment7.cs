...
public class ActionsFulfillment7 : MonoBehaviour
{
    #region Variables
    public string[,] acts;
    public string[] lines;
    ...
    public GameObject[] safeHouseArray;
	...
	void Update ()
    {
        if (waitComplete)
        {
            if (System.IO.File.Exists(Application.dataPath + "/output8.txt")/* && !doneOnce3*/)
            {
                //doneOnce3 = true;
                ...
                else
                {
                    ...
                    {
                        doneOnce2 = true;
                        choice = Random.Range(1, safeHouseArray.Length + 1);
                    }
    ...
    IEnumerator WaitforConstruct()
    {
        ...
        taskComplete = true;
        waitComplete = true;
        acts = this.gameObject.GetComponent<AINavigation7>().acts;
        lines = this.gameObject.GetComponent<AINavigation7>().lines;
        safeHouseArray = GameObject.FindGameObjectsWithTag("SafeHouse");
        string path1 = Application.dataPath + "/output8.txt";
